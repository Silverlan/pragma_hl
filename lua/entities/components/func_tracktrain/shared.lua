...
function ents.FuncTrackTrain:HandleKeyValue(key, val)
	...
	elseif key == "startspeed" then
		local kinematicMover = self:GetEntity():GetComponent(ents.COMPONENT_KINEMATIC_MOVER)
		if kinematicMover ~= nil then
			kinematicMover:SetMaxSpeed(tonumber(val))
		end
	...
	elseif key == "velocitytype" then
		local kinematicMover = self:GetEntity():GetComponent(ents.COMPONENT_KINEMATIC_MOVER)
		if kinematicMover ~= nil then
			kinematicMover:SetVelocityType(tonumber(val))
		end
	elseif key == "orientationtype" then
		local kinematicMover = self:GetEntity():GetComponent(ents.COMPONENT_KINEMATIC_MOVER)
		if kinematicMover ~= nil then
			kinematicMover:SetOrientationType(tonumber(val))
		end
	...
	elseif key == "movesound" then
		local kinematicMover = self:GetEntity():GetComponent(ents.COMPONENT_KINEMATIC_MOVER)
		if kinematicMover ~= nil then
			kinematicMover:SetMoveSoundName(val)
		end
	...
	elseif key == "volume" then
		local kinematicMover = self:GetEntity():GetComponent(ents.COMPONENT_KINEMATIC_MOVER)
		if kinematicMover ~= nil then
			kinematicMover:SetSoundVolume(tonumber(val) / 10.0)
		end
...
function ents.FuncTrackTrain:UpdateNextMoveTarget()
	...
	if closestPointToTarget:Distance(posTgt) > pathRadius then
		return entTgt
	end
	...
	if moveSpeed ~= 0.0 then
		...
		if kinematicMoverComponent ~= nil then
			kinematicMoverComponent:SetSpeed(moveSpeed)
		end
	end

	local ioComponent = ent:GetComponent(ents.COMPONENT_IO)
	if ioComponent ~= nil then
		ioComponent:FireOutput("OnNext", ent)
	end
...
function ents.FuncTrackTrain:GetTargetAngles(entTgt)
	...
	if util.is_valid(entTgt) == false or trComponent == nil then
		return EulerAngles()
	end
	local pos = ent:GetPos()
	pos.y = pos.y - self:GetHeightAboveTrack()
...
function ents.FuncTrackTrain:TeleportToTarget()
	...
	local pathTrackComponent = entTgt:GetComponent(ents.COMPONENT_PATH_TRACK)
	if pathTrackComponent ~= nil then
		pathTrackComponent:OnTeleport(ent)
	end
end
...
function ents.FuncTrackTrain:HandleInput(input, activator, caller, data)
	...
	elseif input == "setspeed" then
		local kinematicMover = self:GetEntity():GetComponent(ents.COMPONENT_KINEMATIC_MOVER)
		if kinematicMover ~= nil then
			kinematicMover:SetSpeed(tonumber(data) * kinematicMover:GetMaxSpeed())
		end
	...
	elseif input == "setspeedreal" then
		local kinematicMover = self:GetEntity():GetComponent(ents.COMPONENT_KINEMATIC_MOVER)
		if kinematicMover ~= nil then
			kinematicMover:SetSpeed(tonumber(data))
		end
...
ents.register_component("func_tracktrain", ents.FuncTrackTrain, "physics", ents.EntityComponent.FREGISTER_BIT_NETWORKED)
