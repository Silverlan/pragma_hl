...
function ents.TriggerChangelevel:ChangeLevel(activator)
	...
	if(util.is_valid(activator) == false) then activator = ent end
	local ioComponent = ent:GetComponent(ents.COMPONENT_IO)
	if(ioComponent ~= nil) then ioComponent:FireOutput("OnChangeLevel",activator) end
...
ents.COMPONENT_TRIGGER_CHANGELEVEL = ents.register_component("trigger_changelevel",ents.TriggerChangelevel)
