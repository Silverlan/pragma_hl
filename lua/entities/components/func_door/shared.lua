...
local defaultMemberFlagsNw = bit.bor(
	defaultMemberFlags,
	ents.BaseEntityComponent.MEMBER_FLAG_BIT_NETWORKED,
	ents.BaseEntityComponent.MEMBER_FLAG_TRANSMIT_ON_CHANGE
)

ents.FuncDoor:RegisterMember("MoveDir", ents.MEMBER_TYPE_EULER_ANGLES, EulerAngles(), {}, defaultMemberFlagsNw)
ents.FuncDoor:RegisterMember("FilterName", ents.MEMBER_TYPE_ENTITY, "", {}, defaultMemberFlags)

ents.FuncDoor:RegisterMember(
	"Open",
	ents.MEMBER_TYPE_BOOLEAN,
	false,
	{},
	bit.bor(defaultMemberFlags, ents.BaseEntityComponent.MEMBER_FLAG_BIT_USE_IS_GETTER)
)
ents.FuncDoor:RegisterMember("StartSound", ents.MEMBER_TYPE_STRING, "", {}, defaultMemberFlags)
ents.FuncDoor:RegisterMember("StopSound", ents.MEMBER_TYPE_STRING, "", {}, defaultMemberFlags)
ents.FuncDoor:RegisterMember("StartCloseSound", ents.MEMBER_TYPE_STRING, "", {}, defaultMemberFlags)
ents.FuncDoor:RegisterMember("CloseSound", ents.MEMBER_TYPE_STRING, "", {}, defaultMemberFlags)
ents.FuncDoor:RegisterMember("DelayBeforeReset", ents.MEMBER_TYPE_FLOAT, 4.0, {}, defaultMemberFlags)
ents.FuncDoor:RegisterMember("Lip", ents.MEMBER_TYPE_FLOAT, 0.0, {}, defaultMemberFlagsNw)
ents.FuncDoor:RegisterMember("BlockingDamage", ents.MEMBER_TYPE_INT32, 0, {}, defaultMemberFlags)
ents.FuncDoor:RegisterMember("ForceClosed", ents.MEMBER_TYPE_BOOLEAN, 0, {}, defaultMemberFlags)
ents.FuncDoor:RegisterMember("IgnoreDebris", ents.MEMBER_TYPE_BOOLEAN, 0, {}, defaultMemberFlags)
ents.FuncDoor:RegisterMember("LockedSound", ents.MEMBER_TYPE_STRING, "", {}, defaultMemberFlags)
ents.FuncDoor:RegisterMember("UnlockedSound", ents.MEMBER_TYPE_STRING, "", {}, defaultMemberFlags)
ents.FuncDoor:RegisterMember(
	"SpawnPosition",
	ents.MEMBER_TYPE_UINT8,
	ents.FuncDoor.SPAWN_POSITION_CLOSED,
	{},
	defaultMemberFlags
)
ents.FuncDoor:RegisterMember(
	"LockedSentence",
	ents.MEMBER_TYPE_UINT8,
	ents.FuncDoor.LOCKED_SENTENCE_NONE,
	{},
	defaultMemberFlags
)
ents.FuncDoor:RegisterMember(
	"UnlockedSentence",
	ents.MEMBER_TYPE_UINT8,
	ents.FuncDoor.UNLOCKED_SENTENCE_NONE,
	{},
	defaultMemberFlags
)
ents.FuncDoor:RegisterMember("MinimumLightLevel", ents.MEMBER_TYPE_STRING, "", {}, defaultMemberFlags)

ents.FuncDoor:RegisterMember("SpawnFlags", ents.MEMBER_TYPE_UINT32, 0, {}, defaultMemberFlags)
...
function ents.FuncDoor:OnEntitySpawn()
	if self:GetSpawnPosition() == ents.FuncDoor.SPAWN_POSITION_OPEN then
		self:SetOpen(true)
	end
end
...
function ents.FuncDoor:OnTargetReached()
	...
	if kinematicMoverComponent == nil or ioComponent == nil then
		return
	end
	local moveState = kinematicMoverComponent:GetMoveState()
	ioComponent:FireOutput(
		(moveState == ents.KinematicMover.MOVE_STATE_FORWARD) and "OnFullyOpen" or "OnFullyClosed",
		ent
	)
end
...
function ents.FuncDoor:HandleKeyValue(key, val)
	if key == "movedir" then
		self:SetMoveDir(angle.create_from_string(val))
	elseif key == "filtername" then
		self:SetFilterName(val)
	elseif key == "speed" then
		local kinematicMover = self:GetEntity():GetComponent(ents.COMPONENT_KINEMATIC_MOVER)
		if kinematicMover ~= nil then
			kinematicMover:SetStartSpeed(tonumber(val))
		end
	elseif key == "noise1" then
		local kinematicMover = self:GetEntity():GetComponent(ents.COMPONENT_KINEMATIC_MOVER)
		if kinematicMover ~= nil then
			kinematicMover:SetStartSound(val)
		end
	elseif key == "noise2" then
		local kinematicMover = self:GetEntity():GetComponent(ents.COMPONENT_KINEMATIC_MOVER)
		if kinematicMover ~= nil then
			kinematicMover:SetStopSound(val)
		end
	elseif key == "startclosesound" then
		self:SetStartCloseSound(val)
	elseif key == "closesound" then
		self:SetCloseSound(val)
	elseif key == "wait" then
		self:SetDelayBeforeReset(tonumber(val))
	elseif key == "lip" then
		self:SetLip(tonumber(val))
	elseif key == "dmg" then
		self:SetBlockingDamage(tonumber(val))
	elseif key == "forceclosed" then
		self:SetForceClosed(toboolean(val))
	elseif key == "ignoredebris" then
		self:SetIgnoreDebris(toboolean(val))
	elseif key == "locked_sound" then
		self:SetLockedSound(val)
	elseif key == "unlocked_sound" then
		self:SetUnlockedSound(val)
	elseif key == "spawnpos" then
		self:SetSpawnPosition(toboolean(val))
	elseif key == "spawnflags" then
		self:SetSpawnFlags(tonumber(val))
	elseif key == "locked_sentence" then
		self:SetLockedSentence(tonumber(val))
	elseif key == "unlocked_sentence" then
		self:SetUnlockedSentence(tonumber(val))
	elseif key == "_minlight" then
		self:SetMinimumLightLevel(val)
	elseif key == "loopmovesound" then
		local kinematicMover = self:GetEntity():GetComponent(ents.COMPONENT_KINEMATIC_MOVER)
		if kinematicMover ~= nil then
			kinematicMover:SetLoopMovingSound(toboolean(val))
		end
	else
...
function ents.FuncDoor:UpdateMoveDistance(bReverse)
	...
	if kinematicMover ~= nil then
		local d = -self:GetLip()
		if bIntersect0 == true then
			d = d + lineExit0
		end
		if bIntersect1 == true then
			d = d + lineExit1
		end
...
function ents.FuncDoor:IsClosed()
...
function ents.FuncDoor:Open()
	...
	if self:IsLocked() then
		...
		if #lockedSound > 0 then
			self:EmitSound(lockedSound)
		end

		local lockedSentence = self:GetLockedSentence()
		if lockedSentence ~= ents.FuncDoor.LOCKED_SENTENCE_NONE then
		end -- TODO

		if ioComponent ~= nil then
			ioComponent:FireOutput("OnLockedUse", self:GetEntity())
		end
		return
	else
		local unlockedSound = self:GetUnlockedSound()
		if #unlockedSound > 0 then
			self:EmitSound(unlockedSound)
		end

		local unlockedSentence = self:GetUnlockedSentence()
		if unlockedSentence ~= ents.FuncDoor.UNLOCKED_SENTENCE_NONE then
		end -- TODO
	end

	if ioComponent ~= nil then
		ioComponent:FireOutput("OnOpen", self:GetEntity())
	end
	self:SetOpen(true)
	self:UpdateMoveDistance()
	local kinematicMover = self:GetEntity():GetComponent(ents.COMPONENT_KINEMATIC_MOVER)
	if kinematicMover ~= nil then
		kinematicMover:StartForward()
	end
end
...
function ents.FuncDoor:Close()
	...
	local ioComponent = self:GetEntity():GetComponent(ents.COMPONENT_IO)
	if ioComponent ~= nil then
		ioComponent:FireOutput("OnClose", self:GetEntity())
	end
	self:SetOpen(false)
	self:UpdateMoveDistance(true)
	local kinematicMover = self:GetEntity():GetComponent(ents.COMPONENT_KINEMATIC_MOVER)
	if kinematicMover ~= nil then
		kinematicMover:StartBackward()
	end
end
...
function ents.FuncDoor:HandleInput(input, activator, caller, data)
	...
	elseif input == "setspeed" then
		...
		local kinematicMover = self:GetEntity():GetComponent(ents.COMPONENT_KINEMATIC_MOVER)
		if kinematicMover ~= nil then
			kinematicMover:SetStartSpeed(tonumber(data))
		end
	else
		return util.EVENT_REPLY_UNHANDLED
	end
...
ents.register_component("func_door", ents.FuncDoor, "physics", ents.EntityComponent.FREGISTER_BIT_NETWORKED)
