...
ents.KinematicMover:RegisterMember(
	"MoveState",
	ents.MEMBER_TYPE_UINT8,
	ents.KinematicMover.MOVE_STATE_IDLE,
	{},
	defaultMemberFlagsNw
)
...
ents.KinematicMover:RegisterMember(
	...
	0.0,
	{},
	bit.bor(defaultMemberFlagsNw, ents.BaseEntityComponent.MEMBER_FLAG_BIT_PROPERTY)
)
ents.KinematicMover:RegisterMember(
	"VelocityType",
	ents.MEMBER_TYPE_UINT8,
	ents.KinematicMover.VELOCITY_TYPE_INSTANTANEOUSLY,
	{},
	defaultMemberFlagsNw
)
ents.KinematicMover:RegisterMember(
	"OrientationType",
	ents.MEMBER_TYPE_UINT8,
	ents.KinematicMover.ORIENTATION_TYPE_NEVER,
	{},
	defaultMemberFlagsNw
)
...
function ents.KinematicMover:UpdateMove(dt)
	...
	if self:GetMoveRelative() then
		self:SetMoveTarget(self:GetMoveTarget() - mvDelta * dt)
	end
...
function ents.KinematicMover:Stop()
	...
	if #stopSound > 0 then
		self:EmitSound(stopSound)
	end
	if self.m_moveSound ~= nil then
		self.m_moveSound:Stop()
	end
...
function ents.KinematicMover:IsMoving()
	return self:GetMoveState() ~= ents.KinematicMover.MOVE_STATE_IDLE
end
...
function ents.KinematicMover:Reverse()
	if self:GetMoveState() == ents.KinematicMover.MOVE_STATE_FORWARD then
		self:SetMoveState(ents.KinematicMover.MOVE_STATE_BACKWARD)
	elseif self:GetMoveState() == ents.KinematicMover.MOVE_STATE_BACKWARD then
		self:SetMoveState(ents.KinematicMover.MOVE_STATE_FORWARD)
	end
end

function ents.KinematicMover:Resume()
	if self.m_lastMoveDirection == nil or self:IsMoving() then
		return
	end
...
ents.register_component("kinematic_mover", ents.KinematicMover, "physics", ents.EntityComponent.FREGISTER_BIT_NETWORKED)

ents.KinematicMover.EVENT_ON_TARGET_REACHED =
	ents.register_component_event(ents.COMPONENT_KINEMATIC_MOVER, "on_target_reached")
