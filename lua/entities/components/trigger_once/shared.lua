...
function ents.TriggerOnce:CanTrigger(ent,physObj)
	if(self.m_bUseFilter) then
		if(util.is_valid(self.m_filter) == false) then return util.EVENT_REPLY_HANDLED,false end
		if(self.m_filter:ShouldPass(ent) == false) then return util.EVENT_REPLY_HANDLED,false end
...
function ents.TriggerOnce:OnEntitySpawn()
	...
	if(#self.m_filterName > 0) then
		...
		local itFilter = ents.iterator({ents.IteratorFilterEntity(self.m_filterName),ents.IteratorFilterComponent(ents.COMPONENT_FILTER_NAME)})
		local entFilter = itFilter()
		if(entFilter ~= nil) then self.m_filter = entFilter:GetComponent(ents.COMPONENT_FILTER_NAME)
		else
			itFilter = ents.iterator({ents.IteratorFilterEntity(self.m_filterName),ents.IteratorFilterComponent(ents.COMPONENT_FILTER_CLASS)})
			entFilter = itFilter()
			if(entFilter ~= nil) then self.m_filter = entFilter:GetComponent(ents.COMPONENT_FILTER_CLASS) end
		end
	end
end
ents.COMPONENT_TRIGGER_ONCE = ents.register_component("trigger_once",ents.TriggerOnce)
