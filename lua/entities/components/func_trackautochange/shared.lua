...
function ents.FuncTrackAutoChange:OnEntitySpawn()
	...
	if self.m_topTrackName ~= nil then
		self:SetTopTrack(
			ents.iterator(
				bit.bor(ents.ITERATOR_FILTER_DEFAULT, ents.ITERATOR_FILTER_BIT_PENDING),
				{ ents.IteratorFilterEntity(self.m_topTrackName) }
			)()
		)
	end
	if self.m_bottomTrackName ~= nil then
		self:SetBottomTrack(
			ents.iterator(
				bit.bor(ents.ITERATOR_FILTER_DEFAULT, ents.ITERATOR_FILTER_BIT_PENDING),
				{ ents.IteratorFilterEntity(self.m_bottomTrackName) }
			)()
		)
	end
end
...
function ents.FuncTrackAutoChange:HandleKeyValue(key, val)
	if key == "height" then
		self:SetTravelAltitude(tonumber(val))
	elseif key == "spawnflags" then
		self:SetSpawnFlags(tonumber(val))
	elseif key == "rotation" then
		self:SetSpinAmount(tonumber(val))
	elseif key == "train" then
		self.m_trainName = val
	elseif key == "toptrack" then
		self.m_topTrackName = val
	elseif key == "bottomtrack" then
		self.m_bottomTrackName = val
	elseif key == "speed" then
		local componentKinematicMover = self:GetEntity():GetComponent(ents.COMPONENT_KINEMATIC_MOVER)
		if componentKinematicMover ~= nil then
			componentKinematicMover:SetSpeed(tonumber(val))
		end
	else
...
function ents.FuncTrackAutoChange:Trigger()
	...
	if
		componentKinematicMover == nil
		or util.is_valid(entBottomTrack) == false
		or util.is_valid(entTopTrack) == false
	then
		return
	end
	local entTrain = self:GetTrainToSwitch()
	if util.is_valid(entTrain) then
		local attachableComponent = entTrain:AddComponent(ents.COMPONENT_ATTACHMENT)
