...
function ents.EnvFade:StartFade()
	...
	if ioComponent ~= nil then
		ioComponent:FireOutput("OnBeginFade", ent)
	end
	if SERVER then
		ent:BroadcastNetEvent(net.PROTOCOL_SLOW_RELIABLE, self.m_netEvFade)
		return
	end
	if util.is_valid(self.m_fadePanel) then
		self.m_fadePanel:SetVisible(true)
	end
...
ents.register_component("env_fade", ents.EnvFade, "rendering", ents.EntityComponent.FREGISTER_BIT_NETWORKED)
