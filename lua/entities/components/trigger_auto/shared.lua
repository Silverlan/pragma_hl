...
function ents.TriggerAuto:OnRemove()
	if util.is_valid(self.m_cbOnMapLoaded) then
		self.m_cbOnMapLoaded:Remove()
	end
end
...
function ents.TriggerAuto:Trigger()
	...
	if ioComponent ~= nil then
		ioComponent:FireOutput("OnTrigger", ent, ents.IOComponent.IO_FLAG_BIT_FORCE_DELAYED_FIRE)
	end
	if self.m_spawnFlags ~= nil and bit.band(self.m_spawnFlags, ents.TriggerAuto.SF_REMOVE_ON_FIRE) ~= 0 then
