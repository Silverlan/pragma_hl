...
function ents.ScriptedSequence:OnRemove()
	for ent,data in pairs(self.m_entityData) do
		if(util.is_valid(data.callback)) then data.callback:Remove() end
	end
end

function ents.ScriptedSequence:BeginSequenceForTarget(entTgt,moveToPosition)
	if(self.m_entityData[entTgt] ~= nil) then
		if(util.is_valid(self.m_entityData[entTgt].callback)) then self.m_entityData[entTgt].callback:Remove() end
	...
	self.m_entityData[entTgt].callback = aiComponent:AddEventCallback(ents.AIComponent.EVENT_ON_SCHEDULE_COMPLETE,function(schedule,result)
		if(self.m_entityData[entTgt] ~= nil and util.is_valid(self.m_entityData[entTgt].callback)) then self.m_entityData[entTgt].callback:Remove() end
		self.m_entityData[entTgt] = nil
		if(#self.m_nextScript > 0) then
			...
			time.create_simple_timer(0.0,function()
				for ent in ents.iterator({ents.IteratorFilterComponent(ents.COMPONENT_SCRIPTED_SEQUENCE),ents.IteratorFilterEntity(nextScript)}) do
	...
	elseif(self.m_moveToType ~= ents.ScriptedSequence.MOVE_TO_POSITION_NO) then
		...
		elseif(self.m_moveToType == ents.ScriptedSequence.MOVE_TO_POSITION_CUSTOM_MOVEMENT and #self.m_customMoveAnim > 0) then
...
function ents.ScriptedSequence:BeginSequence()
	...
	if(ioComponent ~= nil) then ioComponent:FireOutput("OnBeginSequence",ent) end
...
function ents.ScriptedSequence:CancelSequence()
	local ent = self:GetEntity()
	local ioComponent = ent:GetComponent(ents.COMPONENT_IO)
	if(ioComponent ~= nil) then ioComponent:FireOutput("OnCancelSequence",ent) end
	for ent,data in pairs(self.m_entityData) do
		if(ent:IsValid()) then
...
function ents.ScriptedSequence:FindTargets()
	...
	if(self.m_searchRadius ~= 0.0) then table.insert(filters,ents.IteratorFilterSphere(ent:GetPos(),self.m_searchRadius)) end
...
ents.COMPONENT_SCRIPTED_SEQUENCE = ents.register_component("scripted_sequence",ents.ScriptedSequence,ents.EntityComponent.FREGISTER_BIT_NETWORKED)
