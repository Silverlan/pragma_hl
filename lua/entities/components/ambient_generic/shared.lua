...
function ents.AmbientGeneric:HandleKeyValue(key,val)
	if(key == "message") then
		...
		if(sndComponent ~= nil) then sndComponent:SetSoundSource(val) end
	elseif(key == "health") then
		local sndComponent = self:GetEntity():GetComponent(ents.COMPONENT_SOUND)
		if(sndComponent ~= nil) then sndComponent:SetGain(tonumber(val) /10.0) end
	...
	elseif(key == "pitch") then
		local sndComponent = self:GetEntity():GetComponent(ents.COMPONENT_SOUND)
		if(sndComponent ~= nil) then sndComponent:SetPitch(tonumber(val) /100.0) end
	...
	elseif(key == "radius") then
		local sndComponent = self:GetEntity():GetComponent(ents.COMPONENT_SOUND)
		if(sndComponent ~= nil) then sndComponent:SetMaxDistance(tonumber(val)) end
...
function ents.AmbientGeneric:OnEntitySpawn()
	if(self.m_sourceEntityName == nil) then return end
	...
	if(attComponent ~= nil) then
		...
		local ent = it()
		if(ent ~= nil) then attComponent:AttachToEntity(ent) end
	end
end
...
function ents.AmbientGeneric:HandleInput(input,activator,caller,data)
	if(input == "pitch") then
		local sndComponent = self:GetEntity():GetComponent(ents.COMPONENT_SOUND)
		if(sndComponent ~= nil) then sndComponent:SetPitch(tonumber(data) /100.0) end
	...
	elseif(input == "volume") then
		local sndComponent = self:GetEntity():GetComponent(ents.COMPONENT_SOUND)
		if(sndComponent ~= nil) then sndComponent:SetGain(tonumber(data) /10.0) end
...
ents.COMPONENT_AMBIENT_GENERIC = ents.register_component("ambient_generic",ents.AmbientGeneric,ents.EntityComponent.FREGISTER_NONE)
