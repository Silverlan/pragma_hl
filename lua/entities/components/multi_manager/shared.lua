...
function ents.MultiManager:HandleInput(input,activator,caller,data)
	if(input == "trigger") then
		...
		if(ioComponent ~= nil) then ioComponent:FireOutput("OnTrigger",ent) end
	else return util.EVENT_REPLY_UNHANDLED end
	return util.EVENT_REPLY_HANDLED
end
ents.COMPONENT_MULTI_MANAGER = ents.register_component("multi_manager",ents.MultiManager,ents.EntityComponent.FREGISTER_NONE)
