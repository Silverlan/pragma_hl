...
function ents.PathTrack:OnPass(ent)
	...
	if(ioComponent == nil or util.is_valid(ent) == false) then return end
...
function ents.PathTrack:HandleInput(input,activator,caller,data)
	...
	elseif(input == "togglepath") then
		...
		if(toggleComponent ~= nil) then toggleComponent:Toggle() end
	elseif(input == "enablepath") then
		local toggleComponent = self:GetEntity():GetComponent(ents.COMPONENT_TOGGLE)
		if(toggleComponent ~= nil) then toggleComponent:SetTurnedOn(true) end
	elseif(input == "disablepath") then
		local toggleComponent = self:GetEntity():GetComponent(ents.COMPONENT_TOGGLE)
		if(toggleComponent ~= nil) then toggleComponent:SetTurnedOn(false) end
	else return util.EVENT_REPLY_UNHANDLED end
	return util.EVENT_REPLY_HANDLED
end

function ents.PathTrack:GetNextTarget()
	if(self.m_altPathEnabled) then return self.m_entAltTarget end
...
ents.COMPONENT_PATH_TRACK = ents.register_component("path_track",ents.PathTrack,ents.EntityComponent.FREGISTER_BIT_NETWORKED)
