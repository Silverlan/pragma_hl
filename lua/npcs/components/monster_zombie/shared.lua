...
Animation.RegisterActivity("ACT_DIE_HEADSHOT")

Animation.RegisterEvent("EVENT_NPC_BODYDROP_LIGHT")
Animation.RegisterEvent("EVENT_NPC_1")
Animation.RegisterEvent("EVENT_NPC_2")
Animation.RegisterEvent("EVENT_NPC_3")
...
function Component:PlayScriptSound(snd)
	...
	if(snd == nil or animComponent == nil) then return end
	local leftFoot
	if(snd == "NPC.StepLeftWalk" or snd == "NPC.StepLeftRun") then leftFoot = true
	elseif(snd == "NPC.StepRightWalk" or snd == "NPC.StepRightRun") then leftFoot = false end
...
ents.COMPONENT_MONSTER_ZOMBIE = ents.register_component("monster_zombie",Component,ents.EntityComponent.FREGISTER_BIT_NETWORKED)
