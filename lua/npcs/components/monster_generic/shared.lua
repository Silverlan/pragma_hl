...
function ents.MonsterGeneric:HandleKeyValue(key,val)
	if(key == "model") then
		...
		if(mdlComponent ~= nil) then mdlComponent:SetModel(val) end
...
function ents.MonsterGeneric:InitializeAI(component)
	if(SERVER) then component:SetNPCState(ai.NPC_STATE_IDLE) end
end
...
ents.COMPONENT_MONSTER_GENERIC = ents.register_component("monster_generic",ents.MonsterGeneric,ents.EntityComponent.FREGISTER_BIT_NETWORKED)
