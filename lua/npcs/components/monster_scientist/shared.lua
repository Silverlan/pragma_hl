...
function ents.MonsterScientist:InitializeCharacter(component)
	if(SERVER) then component:SetFaction("scientist") end
...
function ents.MonsterScientist:HandleKeyValue(key,val)
	if(key == "body") then
		val = tonumber(val)
		local skin = 0
		if(val == -1) then val = math.random(0,3) end
		local bg = val
		if(val == 2) then skin = 1 end
...
ents.COMPONENT_MONSTER_SCIENTIST = ents.register_component("monster_scientist",ents.MonsterScientist,ents.EntityComponent.FREGISTER_BIT_NETWORKED)
